...
from utils import (change_last_modified, clean_url, escape_path,
                   is_changed_since, is_edited_since, prompt_for_input,
                   setup_cron_job, validate_url)
...
async def download_file(
    ...
    try:
        ...
        logger.error(
            f"Error downloading file {display_name} from {course_name}: {str(e)}")
...
async def get_files_and_download(
    ...
    try:
        ...
        if not isinstance(files, list):
            logger.error(
                f"Unexpected response format for files in folder {folder_name} of course {course_name}. Response: {files}")
            return
        ...
        for file in sorted(files, key=lambda f: f.get(
            ...
            if isinstance(file, dict) and "display_name" in file:
                ...
                logger.warning(
                    f"Unexpected file format in folder {folder_name} of course {course_name}: {file}")

        logger.debug(
            f"{course_name}'s folder {folder_name} has {len(files_to_download)} files to download")
        ...
        logger.error(
            f"Error processing folder {folder_name} in course {course_name}: {str(e)}")


async def process_course(api, course: CourseDict, progress_tracker: ProgressTracker, config: CanvasConfig) -> None:
    try:
        ...
        for folder in folders:
            ...
            if not isinstance(files, list):
                logger.error(
                    f"Unexpected response format for files in course {course['name']}. Response: {files}")
                continue
            ...
            try:
                ...
                logger.error(
                    f"Error processing files for course {course['name']}: {str(e)}")
...
async def setup(config: CanvasConfig) -> None:
    ...
    prompt = f"Enter your Canvas URL[{default_url}]: " if default_url else "Enter your Canvas URL: "
    ...
    token = prompt_for_input(token_prompt,
                             default=config.token) or config.token
    ...
    cron_setup_choice = prompt_for_input(
        "Set up a cron job to run every 2 hours? (y/n) [n]: ",
        default="n"
    ).lower()
    ...
    print(
        f"\nSetup complete! Please run \033[1mpython {script_name} run\033[0m to start tracking.")
